...
Pixel scalar_Pixel(Pixel a, float alpha)
{
    Pixel pix;
    pix.r=std::min(int(a.r*alpha),255);
    pix.g=std::min(int(a.g*alpha),255);
    pix.b=std::min(int(a.b*alpha),255);
    return pix;
}
Pixel sum_Pixel(Pixel a , Pixel c){
    Pixel pix;
    pix.r=std::min(int(a.r+c.r),255);
    pix.g=std::min(int(a.g+c.g),255);
    pix.b=std::min(int(a.b+c.b),255);
    return pix;
}
...
void Image::reverse(){
    for(int i=0;i<width;i++)
        for(int j=0;j<height/2;j++){
            Pixel buf=GetPixel(i,j);
            buf.a=255;
            Pixel buf2=GetPixel(i,height-j-1);
            buf2.a=255;
            PutPixel(i,j,buf2);
            PutPixel(i,height-j-1,buf);
        }
}
